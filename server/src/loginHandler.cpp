...
void LoginHandler::listen() {
  while (_connected) {
    ...
    int new_socket =
        accept(_server_fd, (struct sockaddr *)&_address, (socklen_t *)&addrlen);
    char buffer[1024];
    ...
    if (header.type == Header::HeaderType::LOGIN_REQUEST) {
      ...
      if (!validator.validate(request.username, request.password)) {
        ...
        LoginResponse response =
            LoginResponse(LoginResponse::LoginStatus::FAILED);
      ...
      std::thread t([request, new_socket]() {
        ...
        EchoHandler echoHandler(new_socket, request.username, request.password);
